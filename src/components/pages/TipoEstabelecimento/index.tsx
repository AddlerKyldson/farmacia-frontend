...
const TipoEstabelecimento: React.FC = () => {
    const { user } = useAuth();
    ...
    const handleExcluir = async (id: number) => {
        setConfirm({
            ...
            message: ['Deseja realmente excluir este tipo de estabelecimento?'],
            onConfirm: async () => {
                try {
                    await axios.delete(
                        `${server.url}${server.endpoints.tipo_estabelecimento}/${id}`
                    );
                    ...
                    setAlert({
                        ...
                        success: false,
                        title: 'Erro',
                        message: ['Erro ao excluir o tipo de estabelecimento!'],
                        onConfirm: () => {
                            setAlert({
                                show: false,
                                success: false,
    ...
    return (
        <Layout>
            <Breadcrumb paginas={[{ texto: "Home", href: '/' }, { texto: "TipoEstabelecimento" }]} />

            <Titulo titulo="Tipo Estabelecimento" botao={['1', '2', '8'].includes(user_type) ? { texto: "Cadastrar", href: "/tipos-estabelecimentos/form" } : undefined} />
                        ...
                        {dados.map((dado) => (
                            <tr key={dado.id}>
                                ...
                                {['1', '2', '8'].includes(user_type) && (
                                    <td>
                                        <a href={`/tipos-estabelecimentos/form/${dado.id}`} className="btn btn-warning btn-sm">{"Editar"}</a>
